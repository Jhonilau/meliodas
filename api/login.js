...
export default async function handler(req, res) {
  ...
  const existing = await redis.get(`active:${username}`);
  if (existing) return res.status(403).json({ message: 'Akun sedang digunakan di perangkat lain.' });

  const token = uuidv4();
  await redis.set(`active:${username}`, token);
